...
//Buffer temporario de transmisión
static circ_buff_t transBuff;
...
void MCP25625_interrupt_callback(void)
{
	...
	if(MCP25625_isTbuffer_empty(0,FLAG_RGSTR))
	{
		...
		MCP25625_BIT_MODIFY(CANINTF,TX0IF_MASK,TX0IF_DATA(0));
	}
}
...
void MCP25625_init(int ID, int mask, int filter)
{
	...
	MCP25625_BIT_MODIFY(0x0D,0x07,0x01);
...
bool MCP25625_send(int ID,char * databuffer, int bufflen, int whichbuffer)//por ahora lo hago solo con el buffer 0
{
	...
	MCP25625_READ(TXBXCTRL(whichbuffer),bufferaux,1);
